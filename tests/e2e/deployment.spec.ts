...
test.describe('デプロイ環境の動作確認', () => {
  test('ホームページが正しく表示される', async ({ page }) => {
    ...
    // タイトルの確認
    await expect(page).toHaveTitle(/Visual Prompt Builder/);
    
    // 主要な要素が表示されているか確認
    await expect(page.getByRole('heading', { name: /Visual Prompt Builder/i })).toBeVisible();
  ...
  test('言語切り替えが機能する', async ({ page }) => {
    ...
    if (await langButton.isVisible()) {
      // 現在の言語を確認
      const initialText = await page.getByRole('heading', { name: /Visual Prompt Builder/i }).textContent();
      ...
      // 言語が切り替わったことを確認
      const newText = await page.getByRole('heading', { name: /Visual Prompt Builder/i }).textContent();
