...
test.describe('デプロイ環境の動作確認', () => {
  test('ホームページが正しく表示される', async ({ page }) => {
    ...
    // カードリンクが存在するか確認（aria-labelで識別）
    await expect(page.getByRole('link', { name: 'プロンプト作成を開始' })).toBeVisible();
    await expect(page.getByRole('link', { name: '作成履歴を確認' })).toBeVisible();

    // カード内のテキストも確認
    await expect(page.getByText('新しく作成')).toBeVisible();
  ...
  test('プロンプトビルダーのワークフローが動作する', async ({ page }) => {
    ...
    // 新しく作成リンクをクリック（aria-labelで識別）
    await page.getByRole('link', { name: 'プロンプト作成を開始' }).click();
  ...
  test('言語切り替えが機能する', async ({ page }) => {
    ...
    if (buttonCount > 0) {
      ...
      // 言語切り替え機能が未実装の場合はスキップ
      console.log('言語切り替え機能は未実装のためスキップ');
    }
  });
