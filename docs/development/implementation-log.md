...
- 本番環境で画像が表示されない問題の追加調査と修正
- APIレスポンスのラップ構造とフロントエンドの期待値の不一致を解消

#### 問題の詳細

1. **症状**:
   - 画像URLは正しく生成されている（https://image.kantanprompt.com/generated/...）
   - HTTPステータス200で画像自体はアクセス可能
   - しかしフロントエンドで画像が表示されない

2. **原因分析**:
   - APIは`createSuccessResponse`でレスポンスをラップ：
     ```json
     {
       "success": true,
       "data": { "image": "...", "imageUrl": "..." },
     ...
     ```
   - フロントエンドは直接プロパティにアクセス：
     ```typescript
     const data = await response.json();
     return { image: data.image }; // undefined!
     ```

#### 修正内容

- `frontend/src/services/imageGeneration.ts`を修正
- `responseData.data`にアクセスするように変更
- 後方互換性のためフォールバック処理も追加

#### 完成物

- PR #91を作成
- 本番環境で画像が正しく表示されるように修正完了

#### 教訓

- APIとフロントエンドのインターフェース仕様は必ず一致させる
- レスポンス構造を変更する際は全ての呼び出し側を確認
- 開発時はネットワークタブでレスポンス構造を必ず確認
...
## 2025-07-24 - CUDA Out of Memory エラーの対策
...
- **詳細**:
  - 5.81 GiB を割り当てようとして失敗
  - GPU: 44.52 GiB total capacity
  - 34.97 GiB already allocated
  - 2.47 GiB free
- **発生条件**: スマホの高解像度写真をi2iでアップロード時

### 実施内容

1. **画像生成APIの実装調査**
   - フロントエンド: `ImageStep`では5MB以上の画像を自動リサイズ
   - フロントエンド: `ImageToImage`ページではリサイズ処理が**未実装**
   - バックエンド: 画像サイズの検証はあるがリサイズ機能なし

2. **フロントエンド修正**
   - `ImageToImage.tsx`に画像リサイズ処理を追加
   - 3MB以上の画像を自動的に2048x2048、品質0.8にリサイズ
   - ImageStepより厳しい制限でGPUメモリ使用量を削減

3. **バックエンド修正**
   - `replicate.ts`で全モデル共通の画像サイズチェックを実装
   - 10MB以上の画像はエラー、5MB以上は警告
   - FLUX系モデルのパラメータを最適化:
     - 画質: 90 → 80
     - flux-variations: 1MP → 0.25MP (512x512)
     - flux-fillのsteps上限: 20
     - guidanceScale上限: 5

### 成果物

1. **フロントエンドの改善**
   - スマホ写真が自動的に適切なサイズにリサイズされる
   - ユーザーフレンドリーなリサイズ通知
...
1. **GPUメモリ管理**
   - 高解像度画像処理には大量のGPUメモリが必要
   - 画像サイズとメモリ使用量は指数関数的な関係
   - 事前のリサイズがメモリエラー回避に最も効果的
